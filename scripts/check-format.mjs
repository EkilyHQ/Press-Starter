...
const SUPPORTED_EXTENSIONS = new Set(['.js', '.json', '.mjs', '.yaml', '.yml']);
...
const EXCLUDED_PREFIXES = ['assets/', 'dist/', 'node_modules/', 'wwwroot/'];
  ...
  const checks = await Promise.all(
    paths.map(async (file) => {
      ...
      try {
        ...
        throw new Error(`Prettier could not check ${file}: ${error.message}`, { cause: error });
      }
    })
  );
...
async function main() {
  ...
  if (options.writeBaseline) {
    const files = await unformattedPaths(allCandidates, { tolerateParserErrors: true });
  ...
  const stale = baseline.files.filter((file) => !candidateSet.has(file));
  ...
  process.stdout.write(
    `Prettier passed for ${enforced.length} files; ${baseline.files.length} YAP-owned tooling files remain.\n`
  );
}
