...
if ! grep -F 'system-release.json declares release intent' "${workflow}" >/dev/null; then
  echo "YAP sync workflow must fail when system-release.json declares an intent that cannot be fetched" >&2
  exit 1
fi

if ! grep -F 'refusing legacy fallback' "${workflow}" >/dev/null; then
  echo "YAP sync workflow must fail when it cannot prove a release has no intent" >&2
  exit 1
fi
